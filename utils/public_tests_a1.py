...
def test_eval_mse(target):
    ...
    result = target(y_hat, y_tmp)
    assert np.isclose(
        result, 4.5), f"Wrong value. Expected 4.5, but got {result}. Remember the square termn"

    y_hat = np.array([3.] * 5)
    y_tmp = np.array([2.] * 5)
    result = target(y_hat, y_tmp)
    assert np.isclose(
        result, 0.5), f"Wrong value. Expected 0.5, but got {result}. Remember to divide by (2*m)"
...
def test_eval_cat_err(target):
    ...
    assert not np.isclose(
        result, 6.), f"Wrong value. Expected 1, but got {result}. Did you divided by m?"
    ...
    assert type(
        res_tmp) != np.ndarray, f"The output must be an scalar but got {type(res_tmp)}"
...
def model_test(target, classes, input_size):
    ...
    assert type(
        target.loss) == SparseCategoricalCrossentropy, f"Wrong loss function. Expected {SparseCategoricalCrossentropy}, but got {target.loss}"
    assert type(
        target.optimizer) == Adam, f"Wrong loss function. Expected {Adam}, but got {target.optimizer}"
...
def model_r_test(target, classes, input_size):
    ...
    for layer in target.layers:
        ...
        if not (expected[i][3] == None):
            assert type(
                layer.kernel_regularizer) == expected[i][3][0], f"Wrong regularizer. Expected L2 regularizer but got {type(layer.kernel_regularizer)}"
            assert np.isclose(layer.kernel_regularizer.l2,
                              expected[i][3][1]), f"Wrong regularization factor. Expected {expected[i][3][1]}, but got {layer.kernel_regularizer.l2}"
        else:
