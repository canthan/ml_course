...
def prepare_dataset(filename):
    ...
    x_train, x_, y_train, y_ = train_test_split(
        x, y, test_size=0.40, random_state=80)
...
def train_plot_reg_params(reg_params, x_train, y_train, x_cv, y_cv, degree=1, baseline=None):
    ...
    plt.plot(reg_params, np.repeat(baseline, len(reg_params)),
             linestyle='--', label='baseline')
...
def train_plot_learning_curve(model, x_train, y_train, x_cv, y_cv, degree=1, baseline=None):
    ...
    # Plot the results
    plt.plot(num_samples_train_and_cv, train_mses,
             marker='o', c='r', label='training MSEs')
    plt.plot(num_samples_train_and_cv, cv_mses,
             marker='o', c='b', label='CV MSEs')
    plt.plot(num_samples_train_and_cv, np.repeat(
        baseline, len(percents)), linestyle='--', label='baseline')
