from utils.public_tests_a1 import *
from utils.assigment_utils import *
import numpy as np
...
plt_train_test(X_train, y_train, X_test, y_test,
               x, y_pred, x_ideal, y_ideal, degree)
...
print("X_train.shape:", X_train.shape, "X_cv.shape:",
      X_cv.shape, "X_test.shape:", X_test.shape)
...
def model_predict(Xl): return np.argmax(
    tf.nn.softmax(model.predict(Xl)).numpy(), axis=1)


plt_nn(model_predict, X_train, y_train, classes,
       X_cv, y_cv, suptitle="Complex Model")
...
def model_predict_s(Xl): return np.argmax(
    tf.nn.softmax(model_s.predict(Xl)).numpy(), axis=1)


plt_nn(model_predict_s, X_train, y_train, classes,
       X_cv, y_cv, suptitle="Simple Model")
...
def model_predict_r(Xl): return np.argmax(
    tf.nn.softmax(model_r.predict(Xl)).numpy(), axis=1)


plt_nn(model_predict_r, X_train, y_train, classes,
       X_cv, y_cv, suptitle="Regularized")
