...
import utils.bias_utils as utils
...
utils.train_plot_poly(model, x_train, y_train, x_cv,
                      y_cv, max_degree=10, baseline=400)

# Train and plot polynomial regression models. Bias is defined lower.
utils.train_plot_poly(model, x_train, y_train, x_cv,
                      y_cv, max_degree=10, baseline=250)
...
utils.train_plot_poly(model, x_train, y_train, x_cv,
                      y_cv, max_degree=6, baseline=250)
...
print(
    f"the shape of the entire cross validation set (target) is: {y_cv.shape}\n")
